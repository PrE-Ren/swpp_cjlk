...
const get_label = (kind) => {
  if (kind == 0)
    return <Label as='a' color='yellow' ribbon> 음식배달 </Label>
  else if (kind == 1)
    return <Label as='a' color='orange' ribbon> 택시합승 </Label>
  else if (kind == 2)
    return <Label as='a' color='blue' ribbon> 공동구매 </Label>
  else if (kind == 3)
    return <Label as='a' color='pink' ribbon> 스터디 </Label>
  else if (kind == 4)
    return <Label as='a' color='green' ribbon> 운동 </Label>
  else if (kind == 5)
    return <Label as='a' color='red' ribbon> 미팅 </Label>
  else
    return <Label as='a' color='black' ribbon> 기타 </Label>
}
...
export const KindList = ({ meetinglist_list }) => {
  ...
  if (meetinglist_list != null) {
    ...
    const label_tag = get_label(kind)
    return (
      <Segment raised>
        {label_tag}                               {/* 라벨 */}
