...
export class MeetingCreate extends React.Component{
  ...
  change_map_false = (e) => { this.setState({ map_checked : false }), sessionStorage.setItem("lat", 0), sessionStorage.setItem("lng", 0) }
  change_map_true = (e) => this.setState({ map_checked : true })

  componentDidMount(){
    const info = JSON.parse(sessionStorage.getItem('meeting_info'))

    // 수정할 때, 혹은 미팅 정보를 보려고 할 때
    if (info !== null) {
    ...
    if (meeting_info == null) {
      ...
      return (
        <Form>
          ...
          {this.state.map_checked ? <div><br/><Map meeting_info = {null} write = {true} /></div> : <div><br/></div>}
    ...
    // Modify (수정 버튼을 누르면 세션 스토리지에 해당 미팅 정보가 저장됨)
      ...
      return (
        <Form>
          ...
          {this.state.map_checked ? <div><br/><Map meeting_info = {meeting_info} write = {true} /></div> : <div><br/></div>}
