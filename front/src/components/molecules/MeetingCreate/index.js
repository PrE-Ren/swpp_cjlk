...
const MeetingCreate = ({ state, newReq }) => {
  ...
  return (
    <MeetingCreateCss>
      유형: <input ref={node => {kind=node;}}/><br />
      제목: <input ref={node => {title=node;}}/><br />
      모집 마감 기한 <input type="datetime-local" ref={node => {due=node;}} /><br />
      최소 인원: <input ref={node => {min_people=node;}}/><br />
      최대 인원: <input ref={node => {max_people=node;}}/><br />
      내용: <textarea rows="30" cols="100" ref={node => {description=node;}} /><br />
