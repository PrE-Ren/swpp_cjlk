...
import { JoinList } from '.'

const wrap = (props = {}) => shallow(<JoinList {...props} />)
...
it('renders same text when different children, props passed in', () => {
  ...
  expect(wrapper.contains('Now')).toBe(true)
  expect(wrapper.contains('내가 참여중인 모임')).toBe(true)
  ...
  expect(wrapper.contains('Now')).toBe(true)
  expect(wrapper2.contains('내가 참여중인 모임')).toBe(true)
})

it('renders nothing when null', () => {
  const wrapper = wrap({meetinglist_join : null})
  expect(wrapper.contains('Now')).toBe(false)
  expect(wrapper.contains('내가 참여중인 모임')).toBe(false)
})